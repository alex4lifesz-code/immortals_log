      ...
      <div className="px-2 py-2 shrink-0 space-y-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            className={`w-full bg-ink-dark/70 border border-ink-light/45 rounded-md pl-8 pr-8 ${isMobile ? "py-2.5 text-sm" : "py-1.5 text-[11px]"} text-cloud-white placeholder:text-mist-dark/70 outline-none transition-colors duration-150 focus:border-ink-light/70`}
          />
          {searchTerm && (
            <button
              onClick={() => onSearch("")}
                ...
                elements.push(
                  <div key={exercise.id}>
                    ...
                    >
                      ...
                      <span className={`shrink-0 text-[9px] font-medium px-1 py-0 rounded ${useThemeColor ? 'text-jade-glow' : ''}`} style={!useThemeColor ? { color: sidebarTierInfo.glowColor } : undefined}>
