...
import { useDisplaySettings, TechniqueDisplayMode, ActiveCardStyle, DISPLAY_DEFAULTS } from "@/context/DisplaySettingsContext";
    ...
    return (
      ...
      >
        <div
          ...
          `}
          style={!isSelected && showIllumination && glowIntensity > 0 && glowIntensity < 100 ? { boxShadow: `var(--glow-subtle), ${glowStyle.boxShadow || ''}` } : undefined}
          role="button"
          tabIndex={0}
          onClick={() => onSelect(exercise.id)}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onSelect(exercise.id); } }}
        ...
        >
          ...
          {/* Selected indicator */}
          {isSelected && (
            <div className="absolute -top-1 -right-1 w-4 h-4 bg-jade-glow rounded-full flex items-center justify-center shadow-[var(--glow-jade)]">
              <svg className="w-2.5 h-2.5 text-ink-deep" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
              </svg>
            </div>
        ...
        {isLoadingExercises ? (
          ...
          sortedExercises.map((exercise, index) => (
            <TechniqueCard
              ...
              displayMode={DISPLAY_DEFAULTS.sidebarMode}
              compact={isCompact}
              cardStyle={DISPLAY_DEFAULTS.sidebarStyle}
            />
          ))
        )}
      </div>
